      ...
      {particles.map((p) => {
        ...
        return (
          <motion.div key={p.id}
            ...
            animate={{ x: origin.x + Math.cos(rad) * p.distance, y: origin.y + Math.sin(rad) * p.distance - 30, scale: 0, opacity: 0, rotate: p.rotate }}
            transition={{ duration: 0.7, ease: "easeOut" }}
            style={{ position: "absolute", width: p.size, height: p.size, borderRadius: p.id % 2 === 0 ? "50%" : "2px", background: p.color, boxShadow: `0 0 4px ${p.color}` }}
          />
        );
      })}
    </div>
...
const cityLights = [
  ...
  {x:535,y:12,r:0.9},{x:585,y:16,r:1.3},{x:635,y:21,r:0.8},{x:680,y:27,r:1.1},
];

function Planet() {
  return (
    <div className="absolute bottom-0 left-1/2 pointer-events-none select-none" style={{ width: 900, transform: "translateX(-50%)" }}>
      <div style={{ position: "absolute", bottom: 0, left: 0, width: 900, height: 900, borderRadius: "50%", background: "radial-gradient(ellipse at 50% 35%, rgba(80,100,200,0.05) 0%, transparent 65%)", transform: "translateY(72%)" }} />
      <div style={{ position: "absolute", bottom: 0, left: 40, width: 820, height: 820, borderRadius: "50%", background: "radial-gradient(ellipse at 42% 28%, rgba(50,70,180,0.13) 0%, rgba(15,18,50,0.1) 55%, transparent 72%)", transform: "translateY(72%)" }} />
      <svg width="820" height="80" viewBox="0 0 820 80" style={{ position: "absolute", bottom: "1%", left: 40 }} overflow="visible">
        <defs>
      ...
      {cityLights.map((d, i) => (
        <motion.div key={i} className="absolute rounded-full"
          style={{ left: d.x + 40, bottom: d.y, width: d.r * 2, height: d.r * 2, background: "rgba(255,240,180,0.8)", boxShadow: `0 0 ${d.r * 4}px rgba(255,240,180,0.6)` }}
...
// ✅ Fix: use Variants type + custom prop pattern with type assertion
const fadeUp: Variants = {
  ...
  visible: (i: number) => ({
    ...
    transition: {
      delay: i * 0.1,
      duration: 0.6,
      ease: [0.25, 0.46, 0.45, 0.94] as [number, number, number, number], // cubic-bezier instead of string
    },
  }),
};
...
export function Hero() {
  ...
  useEffect(() => {
    setStars(Array.from({ length: 80 }, (_, i) => ({
      ...
      size: Math.random() * 1.4 + 0.3, delay: Math.random() * 4,
    })));
  }, []);

  const copyEmail = () => {
    navigator.clipboard.writeText("hello@sohamchatterjee.dev");
    setCopied(true);
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-6 pt-28 pb-16">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div style={{ position: "absolute", top: "15%", left: "8%", width: 320, height: 320, borderRadius: "50%", background: "radial-gradient(circle, rgba(99,102,241,0.07) 0%, transparent 70%)", filter: "blur(40px)" }} />
        <div style={{ position: "absolute", top: "20%", right: "10%", width: 280, height: 280, borderRadius: "50%", background: "radial-gradient(circle, rgba(139,92,246,0.06) 0%, transparent 70%)", filter: "blur(40px)" }} />
        <div style={{ position: "absolute", bottom: "25%", left: "30%", width: 400, height: 200, borderRadius: "50%", background: "radial-gradient(circle, rgba(59,130,246,0.05) 0%, transparent 70%)", filter: "blur(60px)" }} />
      </div>

      {/* Stars — client only, no hydration mismatch */}
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-4xl mx-auto">
        ...
        <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible"
          ...
          <span className="text-xs font-medium text-foreground/55">Available to collaborate</span>
        </motion.div>
        ...
        <motion.div custom={5} variants={fadeUp} initial="hidden" animate="visible"
          ...
          {[
            ...
            <div key={stat.label} className={`text-center ${i !== 2 ? "border-r border-border pr-8" : ""}`}>
            ...
            <AnimatePresence>
              {ctaHovered && (
                <motion.span key="ripple"
                  initial={{ scale: 0, opacity: 0.3 }} animate={{ scale: 3, opacity: 0 }}
