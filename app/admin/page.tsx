...
import {
  Mail, BookOpen, FolderOpen, Star, LogOut,
  Check, X, Trash2, Plus, Edit, Send, Eye, EyeOff
} from "lucide-react";
import {
  useAdminMessages, useMarkAsRead,
  useAdminProjects, useCreateProject, useUpdateProject, useDeleteProject,
  useAdminTestimonials, useSendInvite, useUpdateTestimonialStatus, useDeleteTestimonial,
  useLogout,
...
export default function AdminPage() {
  ...
  const { mutate: sendInvite, isPending: sendingInvite, isSuccess: inviteSent, reset: resetInvite } = useSendInvite();
  ...
  const handleProjectSubmit = (e: React.FormEvent) => {
    ...
    if (editingProject) {
      updateProject({ id: editingProject, data }, {
        onSuccess: () => { setProjectModal(false); setEditingProject(null); setProjectForm(emptyProject); }
      });
    } else {
      createProject(data, {
        onSuccess: () => { setProjectModal(false); setProjectForm(emptyProject); }
      });
    }
  };
  ...
  const tabs = [
    ...
    { id: "guestbook", label: "Guestbook", icon: BookOpen, count: guestbook.length },
    { id: "projects", label: "Projects", icon: FolderOpen, count: projects.length },
    { id: "testimonials", label: "Testimonials", icon: Star, count: testimonials.filter((t: any) => t.status === "pending").length },
  ];
    ...
    <div className="min-h-screen pt-24 pb-20 px-4 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-foreground/40 text-sm mt-1">Manage your portfolio content</p>
        </div>
          ...
          <button key={t.id} onClick={() => setTab(t.id as Tab)}
            ...
            {t.count > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-xs ${
                tab === t.id ? "bg-background/20 text-background" : "bg-foreground/10 text-foreground/60"
              }`}>
                {t.count}
              </span>
            )}
      ...
      {tab === "messages" && (
        <div className="flex flex-col gap-3">
          {messages.length === 0 ? (
            <p className="text-foreground/30 text-sm text-center py-12">No messages yet</p>
          ) : (
            messages.map((msg: any) => (
              <div key={msg._id}
                className={`p-5 rounded-2xl border transition-all ${msg.read ? "border-border bg-card/30" : "border-foreground/20 bg-card"}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-sm font-semibold text-foreground">{msg.name}</p>
                      {!msg.read && <span className="w-2 h-2 rounded-full bg-blue-500" />}
                      <p className="text-xs text-foreground/30">{msg.email}</p>
                    </div>
                    <p className="text-sm text-foreground/60 mb-2">{msg.message}</p>
      ...
      {tab === "guestbook" && (
        <div className="flex flex-col gap-3">
          {guestbook.length === 0 ? (
            <p className="text-foreground/30 text-sm text-center py-12">No guestbook entries yet</p>
          ) : (
            guestbook.map((entry: any) => (
              <div key={entry._id} className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card/30">
                <div>
                  <p className="text-sm font-semibold text-foreground">{entry.name}</p>
                  <p className="text-sm text-foreground/60 mt-0.5">{entry.message}</p>
                  <p className="text-xs text-foreground/30 mt-1">
                    {new Date(entry.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                  </p>
                </div>
                <button onClick={() => deleteGuestbook(entry._id)}
      ...
      {tab === "projects" && (
        <div>
          <div className="flex justify-end mb-4">
            <button onClick={() => { setEditingProject(null); setProjectForm(emptyProject); setProjectModal(true); }}
          ...
          <div className="flex flex-col gap-3">
            {projects.length === 0 ? (
              <p className="text-foreground/30 text-sm text-center py-12">No projects yet</p>
            ) : (
              projects.map((project: any) => (
                <div key={project._id} className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card/30">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-sm font-semibold text-foreground">{project.title}</p>
                      {project.featured && (
                        <span className="px-2 py-0.5 rounded-full bg-yellow-500/10 text-yellow-500 text-xs">Featured</span>
                      )}
                    </div>
                    <p className="text-sm text-foreground/50 line-clamp-1">{project.description}</p>
                    <div className="flex flex-wrap gap-1 mt-2">
                      {project.techStack?.slice(0, 4).map((tech: string) => (
                        <span key={tech} className="px-2 py-0.5 rounded-full bg-foreground/5 text-foreground/40 text-xs">{tech}</span>
                      ))}
                    </div>
                  </div>
        ...
        <div>
          ...
          <form onSubmit={handleSendInvite}
            ...
            <input type="email" required value={inviteForm.email}
              ...
              className="flex-1 px-3 py-2.5 rounded-xl border border-border bg-background text-foreground text-sm placeholder:text-foreground/30 focus:outline-none focus:border-foreground/30 transition-colors" />
            <button type="submit" disabled={sendingInvite || inviteSent}
              ...
              <Send size={13} />
              {sendingInvite ? "Sending..." : inviteSent ? "Sent! 🎉" : "Send Invite"}
            </button>
          </form>
          ...
          <div className="flex flex-col gap-3">
            {testimonials.length === 0 ? (
              <p className="text-foreground/30 text-sm text-center py-12">No testimonials yet</p>
            ) : (
              testimonials.map((t: any) => (
                <div key={t._id} className="p-5 rounded-2xl border border-border bg-card/30">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {t.message ? (
                        <>
                          <p className="text-xs text-foreground/40 mb-1">{t.role} at {t.company}</p>
                          <p className="text-sm text-foreground/60">{t.message}</p>
                        </>
                      ) : (
                        <p className="text-xs text-foreground/30 italic">Invite sent — waiting for response</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {t.status === "pending" && t.message && (
                        <>
                          <button onClick={() => updateStatus({ id: t._id, status: "approved" })}
                            className="p-2 rounded-xl text-foreground/30 hover:text-green-500 hover:bg-green-500/10 transition-all cursor-pointer">
                            <Check size={15} />
                          </button>
                          <button onClick={() => updateStatus({ id: t._id, status: "rejected" })}
                            className="p-2 rounded-xl text-foreground/30 hover:text-red-500 hover:bg-red-500/10 transition-all cursor-pointer">
                            <X size={15} />
                          </button>
                        </>
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
              ...
              <div className="bg-card border border-border rounded-2xl p-6 max-h-[85vh] overflow-y-auto">
                ...
                <form onSubmit={handleProjectSubmit} className="flex flex-col gap-3">
                  <input required value={projectForm.title}
                  ...
                  <textarea rows={3} value={projectForm.longDescription}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input value={projectForm.liveUrl}
                      onChange={(e) => setProjectForm({ ...projectForm, liveUrl: e.target.value })}
                      placeholder="Live URL"
                      className="w-full px-3 py-2.5 rounded-xl border border-border bg-background text-foreground text-sm placeholder:text-foreground/30 focus:outline-none focus:border-foreground/30 transition-colors" />
                    <input value={projectForm.githubUrl}
                  ...
                  <input value={projectForm.image}
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-border bg-background text-foreground text-sm placeholder:text-foreground/30 focus:outline-none focus:border-foreground/30 transition-colors" />
                  <div className="grid grid-cols-2 gap-3">
                    <input type="number" value={projectForm.order}
                  ...
                  <div className="flex gap-3 mt-2">
                    ...
                    <button type="submit" disabled={creatingProject || updatingProject}
                      ...
                      {creatingProject || updatingProject ? "Saving..." : editingProject ? "Update" : "Create"}
                    </button>
                  </div>
                </form>
              </div>
